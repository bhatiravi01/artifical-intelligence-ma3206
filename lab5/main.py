...
def extract_policy(V_star, P, R, gamma):
    ...
    for s in range(S):
        best_a = max(
            valid_actions[s], key=lambda a: compute_q(V_star, P, R, gamma, s, a)
        )
        policy[s] = best_a
    return policy
...
def main() -> None:
    ...
    policy = np.array(
        [action_to_idx["Search"], action_to_idx["Wait"], action_to_idx["Wait"]],
        dtype=int,
    )
    ...
    final_policy_pi, final_V_pi, pi_value_history, pi_policy_history = policy_iteration(
        P, R, GAMMA, THETA
    )
    print("\nTask 3.2: Final policy from policy iteration")
    for s in range(S):
